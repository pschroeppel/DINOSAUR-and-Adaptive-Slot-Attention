...
#
# Modified 2025 by Philipp Schroeppel. Modifications include: remove things that are not required
...
def put_grid(name, grid, step=None):
    """Setter function to place tensors into the queue to be written out

    Args:
        name: name of grid
        grid: grid visualization to write out
        step: step associated with grid
    """
    EVENT_STORAGE.append({"name": name, "write_type": EventType.GRID, "event": grid, "step": step})
...
def setup_writers(log_wandb, log_tensorboard, max_iterations=None, tensorboard_logs_dir=None, wandb_logs_dir=None, exp_id=None, config=None, comment=None, project="reproduce-adaslot"):
...
class Writer:
    ...
    def write_histogram(self, name, values, step):
        """Required method to write a histogram of values to the logger

        Args:
            name: data identifier
            values: list of values for histogram
            step: the time step to log
        """
        raise NotImplementedError

    @abstractmethod
    def write_grid(self, name, grid, step):
        """method to write out grid visualization
...
class WandbWriter(Writer):
    ...
    def __init__(self, log_dir, project="npcd", exp_id=None, config=None, comment=None):
        exp = wandb.init(project=project, dir=str(log_dir), reinit=True, resume=True, config=config, name=exp_id, notes=comment)

        # Note: the following is only for comparisons to logs from the original DINOSAUR codebase, which uses lightning
        exp.define_metric("trainer/global_step")
        exp.define_metric("*", step_metric="trainer/global_step", step_sync=True)
    ...
    def write_tensor(self, name, tensor, step, **kwargs):
        ...
        # Note: the following is only for comparisons to logs from the original DINOSAUR codebase, which uses lightning
        wandb.log({name: wandb.Image(img), "trainer/global_step": step if step is not None else 0})
    ...
    def write_scalar(self, name, scalar, step):
        ...
        # Note: the following is only for comparisons to logs from the original DINOSAUR codebase, which uses lightning
        wandb.log({name: scalar, "trainer/global_step": step if step is not None else 0})

    def write_histogram(self, name, values, step):
        # wandb.log({name: wandb.Histogram(values)}, step=step if step is not None else 0)

        # Note: the following is only for comparisons to logs from the original DINOSAUR codebase, which uses lightning
        wandb.log({name: wandb.Histogram(values), "trainer/global_step": step if step is not None else 0})
...
class TensorboardWriter(Writer):
    ...
    def write_grid(self, name, grid, step):
        img = grid.vis(out_format={'type': 'np', 'mode': 'RGB', 'dtype': 'uint8'})
        self.writer.add_image(name, img, step if step is not None else 0)
